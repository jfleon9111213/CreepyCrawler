...
function doSearch(search_count, search_market, search_lang, search_safe){
    ...
    function translateWords(wordList) {
        ...
        translateRequest.onreadystatechange = function () {
            if (translateRequest.readyState == 4) {
                ...
                translatedWords = translatedWords.replace(/\+/g, " ");
                elasticSearch(translatedWords);
            }
        }
    }

    /**
     * Sends a POST request to Elasticsearch and uses a more like this query to get back
     * relevant results.
     */
    function elasticSearch(wordQueue) {
        ...
        esRequest.open("POST", "http://elasticsearchpolyglot.eastus.cloudapp.azure.com:9200/classify/_search", true);
        ...
        esRequest.onreadystatechange = function () {
            if (esRequest.readyState == 4) {
            	...
            	*/
                ...
                var nameList = response.match(/\"_id\":\"(.*?)\"/g);
                var snippetList = response.match(/\"content\":\"(.*?)\"/g);

                formatStrings(nameList, "_id:");
                formatStrings(snippetList, "content:");
                ...
                var urlList = nameList;
                var displayUrlList = nameList;

                for (i = 0; i < snippetList.length; i++) {
                    snippetList[i] = snippetList[i].substring(0, 200);
                ...
                translateSnippets(nameList, snippetList, urlList, displayUrlList);
            }
        }
    }
